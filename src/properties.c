...
make_dock()
{
  ...
  if (atom_reply) {
    ...
    if (1) {
      atom_cookie = xcb_intern_atom(connection,
                                    0,
                                    strlen("_NET_WM_WINDOW_TYPE_DOCK"),
                                    "_NET_WM_WINDOW_TYPE_DOCK");
    } else {
      atom_cookie = xcb_intern_atom(connection,
                                    0,
                                    strlen("_NET_WM_WINDOW_TYPE_DIALOG"),
  ...
  atom_cookie = xcb_intern_atom(
    connection, 0, strlen("_NET_WM_DESKTOP"), "_NET_WM_DESKTOP");
  atom_reply = xcb_intern_atom_reply(connection, atom_cookie, NULL);
...
set_title()
{
  xcb_icccm_set_wm_name(
    connection, window, XCB_ATOM_STRING, 8, strlen("I-ZIRCON64"), "I-ZIRCON64");
}
...
make_unresizable()
{
  ...
  xcb_icccm_size_hints_set_min_size(&window_size_hints, information.width, information.height);
  xcb_icccm_size_hints_set_max_size(&window_size_hints, information.width, information.height);
  xcb_icccm_size_hints_set_win_gravity(&window_size_hints, XCB_GRAVITY_CENTER);
  xcb_icccm_set_wm_size_hints(
    connection, window, XCB_ATOM_WM_NORMAL_HINTS, &window_size_hints);
}
